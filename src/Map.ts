...
export const BLOCK_DEFS: Record<BlockId, BlockDef> = {
  ...
  campfire:         { id: "campfire",         name: "Campfire",         color: 0xff6600,                    hardness: 1.0,  placeable: false, transparent: true  },
};
    ...
    for (let lx = 0; lx < CHUNK_SIZE; lx++) {
      for (let ly = 0; ly < WORLD_HEIGHT; ly++) {
        for (let lz = 0; lz < CHUNK_SIZE; lz++) {
          ...
          if (id === "campfire") continue; // rendered as dedicated 3D mesh, not chunk geometry
